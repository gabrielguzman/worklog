...
const hoursInFocus = computed(() => {
    return (useProps().stats.total_focus_minutes / 60).toFixed(1)
})

const completionRate = computed(() => {
    const total = useProps().stats.total_tasks
    if (total === 0) return 0
    return Math.round((useProps().stats.completed_tasks / total) * 100)
})

function useProps() {
    return { stats, productivityChart, topProjects, badges, recentActivity }
}
</script>
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <p v-if="stats.streak_days > 0" class="text-sm text-blue-100 mt-2">
                                    🔥 Racha de {{ stats.streak_days }} días
                                </p>
                            </div>
                        </div>
                        <button
            ...
            <div v-if="activeTab === 'overview'" class="space-y-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
                        ...
                        <p class="text-xs text-gray-500 mt-1">{{ stats.focus_sessions }} sesiones</p>
                    </div>
                    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
                        <p class="text-xs text-gray-600 dark:text-gray-400 uppercase font-medium">Entradas registradas</p>
                        <p class="text-3xl font-bold text-blue-600 dark:text-blue-400 mt-2">{{ stats.total_entries }}</p>
                    </div>
                    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
                        <p class="text-xs text-gray-600 dark:text-gray-400 uppercase font-medium">Proyectos activos</p>
                        <p class="text-3xl font-bold text-green-600 dark:text-green-400 mt-2">{{ stats.total_projects }}</p>
                    </div>
                </div>

                <!-- Badges -->
                <div v-if="badges.length > 0" class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">🏆 Logros Desbloqueados</h2>
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                        <div v-for="badge in badges" :key="badge.id" class="text-center p-4 rounded-lg bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 border border-yellow-200 dark:border-yellow-700">
                ...
                <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
                    ...
                    <div class="space-y-3">
                        <div v-for="(activity, idx) in recentActivity" :key="idx" class="flex items-start gap-4 pb-3 border-b border-gray-100 dark:border-gray-700 last:border-b-0">
                            <div :class="['text-xl', {
            ...
            <div v-if="activeTab === 'stats'" class="space-y-6">
                ...
                <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
                    ...
                    <div class="grid grid-cols-7 gap-2">
                        <div v-for="(day, idx) in productivityChart" :key="idx" class="text-center">
                ...
                <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
                    ...
                    <div class="space-y-3">
                        <div v-for="project in topProjects" :key="project.name" class="flex items-center justify-between">
